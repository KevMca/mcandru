...
So the example in Fig 1.1 would rotate at the same frequency.
...
*__Fig 1.2__ The actual phase voltages of a BLDC *
...
since the position of the sine wave is dependent on the exact position of the rotor. There are two methods
...
The three sensors are placed at one of the phase wires. Therefore, three square waves are produced as the motor
...
be driven for a periodic, which reduces the maximum torque output of the motor. Secondly, the motor
cannot sense its position without moving. Therefore, when the motor is stationary, the motor has to
either move in random direction to figure out it's initial position, or activate one of the coils until
...
field and the signal would ideally be sinusoidal. FOC is needed for this type of control, but that is 
explained in the next section. This form of control acts like if carrots were placed in front of a
...
Now that the ABC bases are represented, we can transform these bases into a set of basis that contains
...
motor and arranged. The phase currents from the motor are converted to the 2D bases using the
Clarke trasnformation. Then, the Park transformation is used to rotate the frame of reference to
that of the motor. The rotor angle is estimated by an observer.
...
to 3 phase voltages using the inverse Clarke and Park trasnformations. A PWM generator creates and
rough estimate of these voltages. The inverter then drives the motor and the cycle repeats.

### ENOUGH OF THE THEORY! 📚

For further reading about how to setup FOC on a VESC, look at this link.
