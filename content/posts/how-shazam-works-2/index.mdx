...
In part 1 of this review I discussed Shazam's method of creating a unique fingerprint
...
of the recorded clip over the entire song for a match.

Let's say we record a 10-second
clip of a song, there are on average 3 features per 0.1 seconds for a total of 10/0.1 = 100
features in the 10 second clip. Each segment of a song compared is (300 * 300) = 90,000
...
Shazam make a very clever use of a hash table by creating a relatively unique hash for 
...
This is then encoded in a 32-bit integer with:
...
I will talk more about the anchor point and what it is later in the article but for
now we'll just see that it is itself a peak in the spectrogram that is selected in
...
In order to understand how the anchor point is chosen for points, we have to understand
the concept of __target zones__. Target zones were first referred to in the white
paper and they are simply a method of splitting the filtered spectrogram into overlapping
groups of points. Let's say for this example that we create target zones of 10 peaks.
...
![](./images/target-zones.jpg "Fig 1: Target zoning example")
...
frequency in each FFT as seen in *Fig 1* above.
...
![](./images/anchor-point-target-zone.png "Fig 2: Target zone anchor point")

*Fig 2* above shows an example of an anchor point for a target zone and how it is
used with every point in the target zone to create a hash for each point.
